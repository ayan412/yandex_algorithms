...
func main() {
	...
	// Основной расчет основан на подборе значений i, чтобы попасть сначала в зн-я p2,n2 - так будет гарантия правильности кол-ва кв-р на этаж. А затем уже искать зн-я для p1,n1
	for i := int64(1); i <= MAXN; i++ { // Пусть НА КАЖДОМ ЭТАЖЕ i квартир
		inp := m * i               // Тогда всего квартир в подъезде
		tp := ((k2 - 1) / inp) + 1 // Предполагаемый номер подъезда p2
		tk := k2 - (tp-1)*inp      // Предполагаемый номер квартиры k2
		tn := ((tk - 1) / i) + 1   // Предполагаемый номер этажа n2

		if tp == p2 && tn == n2 { // Если совпали этаж и подъезд для k2...
			tp = ((k1 - 1) / inp) + 1 // теперь уже  Предполагаемый номер подъезда p1
			tk = k1 - (tp-1)*inp      // Предполагаемый номер квартиры k1
	...
	// Сохраняем результат
	// Ноль соот-т тому что никакие данные i не привели к возможному зн-ю p1, значит входные данные противоречивы
	if vp == 0 {
		fmt.Fprint(out, "-1 ")
	}
	// Если больше 1, значит входные данные не дают точно определить Р
	if vp > 1 {
		fmt.Fprint(out, "0 ")
	}
	if vp == 1 {
	...
	// Если больше 1, значит входные данные не дают точно определить N
	if vn > 1 {
		fmt.Fprint(out, "0")
	}
	if vn == 1 {
